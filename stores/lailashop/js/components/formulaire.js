...
class OrderFormulaire {
  constructor(productId, productData, options = {}) {
    ...
    this.deliveryType = 'domicile'; // 'domicile' or 'bureau'
    this.deliveryPrices = {
      domicile: 700,      // التوصيل للمنزل
      bureau: 500   // التوصيل لمكتب الشحن
    };
    ...
    return `
      <div class="formulaire-wrapper">
        ...
        <form id="orderFormulaire" class="formulaire-form">
          ...
          <div class="formulaire-group">
            <label for="formEmail">البريد الإلكتروني (اختياري)</label>
            <input 
              type="email" 
              id="formEmail" 
              name="email"
            ...
            <div class="formulaire-delivery-options">
              <label class="formulaire-delivery-option">
                <input 
                  ...
                  value="domicile" 
                  checked
                  data-delivery-type="domicile"
                />
                <span class="delivery-label">التوصيل للمنزل</span>
                <span class="delivery-price">700 دج</span>
              </label>
              <label class="formulaire-delivery-option">
                <input 
                  type="radio" 
                  name="delivery_type" 
                  value="bureau"
                  data-delivery-type="bureau"
                />
                <span class="delivery-label">التوصيل لمكتب الشحن</span>
                <span class="delivery-price">500 دج</span>
              </label>
            </div>
          </div>
          ...
          <div class="formulaire-summary">
            ...
            <div class="formulaire-summary-row">
              <span id="summaryShipping">700 دج</span>
  ...
  attachEventListeners() {
    ...
    // Wilaya change listener
    const wilayaSelect = document.getElementById('formWilaya');
    if (wilayaSelect) {
      wilayaSelect.addEventListener('change', () => {
        this.updateDeliveryPriceByWilaya(wilayaSelect.value);
      });
    }
  ...
  updateDeliveryPriceByWilaya(wilayaCode) {
    // Basic delivery calculation inspired by Algerian shipping rates
    // 16 is Algiers typically cheaper
    const basePrice = (wilayaCode === '16') ? 40 : 60;
    
    // Update local delivery prices based on wilaya
    this.deliveryPrices = {
      domicile: basePrice + 30, // Home delivery (+30)
      bureau: basePrice // Office/Desk delivery
    };
    
    // Update labels in UI
    const domicileLabel = document.querySelector('label.formulaire-delivery-option:nth-child(1) .delivery-price');
    const bureauLabel = document.querySelector('label.formulaire-delivery-option:nth-child(2) .delivery-price');
    
    if (domicileLabel) domicileLabel.textContent = `${this.deliveryPrices.domicile}0 دج`;
    if (bureauLabel) bureauLabel.textContent = `${this.deliveryPrices.bureau}0 دج`;

    // To make it look more like DZD (e.g. 70 -> 700)
    this.deliveryPrices.domicile *= 10;
    this.deliveryPrices.bureau *= 10;
  ...
  async handleSubmit(e) {
    ...
    const email = document.getElementById('formEmail')?.value.trim() || `${phone}@guest.com`;
    ...
    // If delivery type is 'domicile', address is required
    if (this.deliveryType === 'domicile' && !address) {
      this.showError('الرجاء إدخال العنوان للتوصيل للمنزل');
      return;
    }

    // Get current delivery price
    const currentDeliveryPrice = this.deliveryPrices[this.deliveryType] || this.deliveryPrices.domicile;
    ...
    const orderPayload = {
      customerInfo: {
        ...
        phone,
        wilaya,
        address: this.deliveryType === 'domicile' ? `${municipality}, ${address}` : `${municipality} (Bureau)`,
        deliveryType: this.deliveryType,
        deliveryPrice: currentDeliveryPrice
      },
      products: [
        {
          product: this.product.id || this.product._id,
    ...
    try {
      ...
      const response = await fetch(`${window.API_BASE_URL || 'http://localhost:8080'}/api/orders`, {
        method: 'POST',
        headers: {
      ...
      this.showError('حدث خطأ في إرسال الطلب: ' + error.message);
    } finally {
